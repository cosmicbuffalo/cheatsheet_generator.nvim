...
function M._add_generation_info(lines, config)
  ...
  if config and config.output and config.output.runtime_note and config.output.runtime_note.enabled then
    ...
    local note_text =
      "> This cheatsheet does not include keymaps added automatically by configured plugins at runtime, such as those from most legacy vim plugins. To see all keymaps available in your current Neovim session, use the `:map` command"

    if config.output.runtime_note.keymap_search then
      note_text = note_text
        .. ", or the `"
        .. config.output.runtime_note.keymap_search
        .. "` keymap to open a fuzzy search for keymaps"
    end
...
function M._generate_sections(lines, categories, prefix_groups, config)
  ...
  for _, prefix in ipairs(prefix_sections) do
    ...
    local section =
      M._generate_section_table(category_data.keymaps, prefix, prefix .. ":", category_data.disabled, config)
    for _, line in ipairs(section) do
      table.insert(lines, line)
    end
  end
  ...
  for _, category in ipairs(plugin_sections) do
    ...
    local section =
      M._generate_section_table(category_data.keymaps, category, strip_prefix, category_data.disabled, config)
    for _, line in ipairs(section) do
      table.insert(lines, line)
    end
  end
...
function M._generate_section_table(keymaps, title, strip_prefix, is_disabled, config)
  ...
  for _, km in ipairs(consolidated) do
    ...
    local source_formatted = utils.format_source_with_links(km.source, github_info, km.line_number, km.repo_info)
