  ...
  for plugin_name, keymaps in pairs(manual_keymaps) do
    ...
    for i, keymap in ipairs(keymaps) do
      ...
      if not keymap.keymap or type(keymap.keymap) ~= "string" then
        return false,
          string.format('config.manual_keymaps["%s"][%d].keymap is required and must be a string', plugin_name, i)
      end

      if not keymap.mode or type(keymap.mode) ~= "string" then
        return false,
          string.format('config.manual_keymaps["%s"][%d].mode is required and must be a string', plugin_name, i)
      end

      if not keymap.desc or type(keymap.desc) ~= "string" then
        return false,
          string.format('config.manual_keymaps["%s"][%d].desc is required and must be a string', plugin_name, i)
      end

      -- Optional fields
      if keymap.source and type(keymap.source) ~= "string" then
        return false,
          string.format('config.manual_keymaps["%s"][%d].source must be a string if provided', plugin_name, i)
      end
    end
  end
