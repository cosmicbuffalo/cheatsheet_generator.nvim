  ...
  if info and info.source then
    ...
    if source:sub(1, 1) == "@" then
      ...
      if source:find(cwd, 1, true) == 1 then
        ...
        -- Clean up any remaining relative path prefixes
        source = source:gsub("^%./", "")
        return source
      else
        -- Handle absolute paths that might need normalization
...
-- @param github_info table|nil GitHub repository information for config repo
-- @param line_number number|nil Line number for the link
-- @param repo_info table|nil Plugin repository information (for plugin sources)
-- @return string Formatted source with link if applicable
function M.format_source_with_links(source, github_info, line_number, repo_info)
  ...
  -- Handle plugin repository sources (URLs that contain github.com)
  if source:match("^https://github%.com/") and source:match("/lazy%.lua$") then
    local github_url, file_path = source:match("^(https://github%.com/[^/]+/[^/]+)/(.*)")
    if github_url and file_path then
      -- Extract username/repo from GitHub URL
      local username, repo = github_url:match("https://github%.com/([^/]+)/([^/]+)")
      local display_name = username and repo and (username .. "/" .. repo) or file_path

      local link_url = github_url .. "/blob/main/" .. file_path
      if line_number and line_number > 0 then
        link_url = link_url .. "#L" .. line_number
        return "[`" .. display_name .. "`](" .. link_url .. ")"
      else
        return "[`" .. display_name .. "`](" .. link_url .. ")"
      end
    end
  ...
  -- Handle plugin repository sources using repo_info
  if repo_info and repo_info.github_url and source:match("/lazy%.lua$") then
    -- Extract username/repo from GitHub URL
    local username, repo = repo_info.github_url:match("https://github%.com/([^/]+)/([^/]+)")
    local display_name = username and repo and (username .. "/" .. repo) or "lazy.lua"

    local link_url = repo_info.github_url .. "/blob/main/lazy.lua"
    if line_number and line_number > 0 then
      link_url = link_url .. "#L" .. line_number
      return "[`" .. display_name .. "`](" .. link_url .. ")"
    else
  ...
  -- Handle config repository sources (existing behavior)
