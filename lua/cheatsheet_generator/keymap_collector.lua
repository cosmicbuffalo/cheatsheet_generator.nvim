...
function M._should_skip_keymap(source, lhs, desc)
  return (source and source:match("lua/personal/")) or lhs:match("^<Plug>") or desc:match("<Plug>")
end
  ...
  for _, keymap in ipairs(all_keymaps) do
    ...
    if not existing then
      ...
      if
        ...
        or (
          current_priority == existing_priority
          and keymap.built_in_section
          and (not existing.built_in_section or keymap.built_in_order < existing.built_in_order)
        )
      then
